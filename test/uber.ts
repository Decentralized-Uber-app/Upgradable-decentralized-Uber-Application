...
describe("Uber", () => {
  async function deployLoadFixture() {
    ...
    const transferToken = await ubertoken
      .connect(admin)
      .transferFromContract(passenger3.address, amt);
    ...
    await uberProxyInteract
      .connect(admin)
      .grantRole(REVIEWER_ROLE, reviewer.address);
  ...
  describe("DriversRegistration", () => {
    it("Should register the driver and revert if driver has registered before", async function () {
      const { uberProxyInteract, driver } = await loadFixture(
        deployLoadFixture
      );
      await uberProxyInteract
        .connect(driver)
        .driversRegister("Ayomide", 12345675);

      await expect(
        uberProxyInteract.connect(driver).driversRegister("Ayomide", 12345675)
  ...
  describe("PassengerRegistration", () => {
    it("Should register the passenger and revert if the passenger has registered before", async function () {
      const { uberProxyInteract, passenger } = await loadFixture(
        deployLoadFixture
      );
      ...
      await expect(
        uberProxyInteract.connect(passenger).passengerRegistration()
      ).to.be.revertedWith("already registered");
    });
  });
  ...
  describe("ReviewDrivers", () => {
    it("Should approve the driver if driver has registered and met the requirements", async function () {
      ...
      await expect(
        uberProxyInteract.connect(reviewer).reviewDriver(driver.address)
      ).to.be.revertedWith("driver already approved");
    });
  });
  ...
  describe("CheckRoles", () => {
    it("Should revert if the address has not been granted REVIEWER_ROLE", async function () {
      ...
      const AccessControl = await ethers.getContractAt(
        ...
        uberProxyInteract.address
      );

      await AccessControl.connect(admin).grantRole(
        REVIEWER_ROLE,
        reviewer2.address
      );

      const HasRole = await AccessControl.connect(admin).hasRole(
        REVIEWER_ROLE,
        reviewer2.address
      );
      ...
      await expect(
        uberProxyInteract.connect(reviewer).reviewDriver(driver.address)
      ).revertedWith("driver already approved");
    });
  });

  describe("OrderRide", () => {
    it("Should allow passenger to order a ride successfully", async function () {
      const { uberProxyInteract, reviewer, driver2, passenger3, ubertoken } =
        await loadFixture(deployLoadFixture);
      ...
      await uberProxyInteract
        .connect(driver2)
        .driversRegister("Sayrarh", 7875322);

      await uberProxyInteract.connect(reviewer).reviewDriver(driver2.address);
      await uberProxyInteract
        .connect(passenger3)
        .orderRide(driver2.address, 2345);

      await expect(
        uberProxyInteract.connect(passenger3).orderRide(driver2.address, 2345)
      ).revertedWith("you have active request");
    });
  });
  ...
  describe("DriverAcceptRide", () => {
    it("Should revert if no ride was requested", async function () {
      ...
      await expect(
        uberProxyInteract.connect(driver2).driverAcceptRide()
  ...
  describe("EndRide", () => {
    it("Should end a ride that already started and revert if no active ride", async function () {
      const { uberProxyInteract, reviewer, driver, driver2, passenger3 } =
        await loadFixture(deployLoadFixture);
      ...
      await uberProxyInteract
        .connect(passenger3)
        .orderRide(driver.address, 2345);

      await uberProxyInteract.connect(driver).driverAcceptRide();

      await uberProxyInteract.connect(driver).endride();
      await expect(
        uberProxyInteract.connect(driver).endride()
      ).to.be.revertedWith("you have no active ride");
    });
  });
});
